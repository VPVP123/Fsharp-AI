        ...
        public static Tuple<int, int> GetAIMove(byte[,] board, int depth, byte tile)
        {
            ...
            if (validMoves.Count > 0)
            {
                ...
                foreach (Tuple<int, int> move in validMoves)
                {
                    ...
                    if (tile == Black)
                    {
                        nodeScore = runMiniMax(childBoard, depth - 1, int.MinValue, int.MaxValue, OtherTile(tile), false);
                    ...
                    {
                        nodeScore = runMiniMax(childBoard, depth - 1, int.MinValue, int.MaxValue, OtherTile(tile), true);
                        if (nodeScore < bestScore)
                        {
                            bestScore = nodeScore;
                            bestMove = move;
